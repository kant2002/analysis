...
/-- Визначення 5.6.1 (Exponentiating a real by a natural number). Here we use the
...
/-- Визначення 5.6.2 (Exponentiating a real by an integer). Here we use the Mathlib definition coming from `DivInvMonoid`. -/
